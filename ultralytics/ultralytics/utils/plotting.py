...
class Annotator:
    ...
    # def __init__(self, im, line_width=5, font_size=5, font="Arial.ttf", pil=False, example="abc"):
    ...
    #             size = font_size or max(round(sum(self.im.size) / 2 * 0.035), 25)
    ...
    #         self.sf = self.lw / 1  # font scale
    def __init__(self, im, line_width=4, font_size=None, font="Arial.ttf", pil=False, example="abc", font_scale=None,
                 thickness=None):
        """Initialize the Annotator class with image and line width along with color palette for keypoints and limbs."""
        non_ascii = not is_ascii(example)  # non-latin labels, i.e. asian, arabic, cyrillic
        input_is_pil = isinstance(im, Image.Image)
        ...
        if self.pil:  # use PIL
            ...
            try:
                font = check_font("Arial.Unicode.ttf" if non_ascii else font)
                size = font_size or max(round(sum(self.im.size) / 2 * 0.035), 20) if font_scale is None else font_scale
                self.font = ImageFont.truetype(str(font), size)
            except Exception:
                self.font = ImageFont.load_default()
            # Deprecation fix for w, h = getsize(string) -> _, _, w, h = getbox(string)
            ...
            assert im.data.contiguous, "Image not contiguous. Apply np.ascontiguousarray(im) to Annotator input images."
            self.im = im if im.flags.writeable else im.copy()
            self.tf = max(self.lw - 1, 2) if thickness is None else thickness  # font thickness
            self.sf = self.lw / 1 if font_scale is None else font_scale  # font scale
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
...
def plot_images(
    ...
    annotator = Annotator(mosaic, line_width=round(fs / 5), font_size=fs, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
