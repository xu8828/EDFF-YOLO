...
from .block import DFL, BNContrastiveHead, ContrastiveHead, Proto, Upsample
...
class SBA(nn.Module):
    ...
    def __init__(self, inc, input_dim=64, num=32):
        super().__init__()
        ...
        self.fc1 = nn.Conv2d(inc, input_dim // 2, kernel_size=1, bias=False)
        self.fc2 = nn.Conv2d(inc, input_dim // 2, kernel_size=1, bias=False)

        self.group1 = nn.GroupNorm(num_groups=num, num_channels=input_dim//2)
        self.group2 = nn.GroupNorm(num_groups=num, num_channels=input_dim // 2)
    ...
    def forward(self, x):
        ...
        L_feature = self.group1(self.d_in1(L_feature))
        H_feature = self.group2(self.d_in2(H_feature))
...
class SDED(nn.Module):
    ...
    def __init__(self, nc=10, hidc=256, ch=()):
        """Initializes the YOLOv8 detection layer with specified number of classes and channels."""
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(ch)  # number of detection layers
        ...
        self.glu = ConvolutionalGLU(hidc, hidc)
