...
class AFF(nn.Module):
    ...
    def __init__(self, channels=64, r=4):
        ...
        self.local_att = nn.Sequential(
            ...
            nn.Conv2d(inter_channels, channels, kernel_size=1, stride=1, padding=0),  # 注意这里的 channels 应该是输出通道数
            nn.BatchNorm2d(channels),  # 这里的 channels 应该是与输出通道数匹配
        )
...
class HFFM(nn.Module):
    def __init__(self, inc, channels=64, r=4) -> None:
        super().__init__()
...
class SBA(nn.Module):
    def __init__(self, inc, input_dim=64):
        ...
        self.fc1 = nn.Sequential(
            nn.Conv2d(inc[1], input_dim // 2, 1, bias=False),
            nn.BatchNorm2d(input_dim // 2),
            nn.ReLU(inplace=True)
        )
        self.fc2 = nn.Sequential(
            nn.Conv2d(inc[0], input_dim // 2, 1, bias=False),
            nn.BatchNorm2d(input_dim // 2),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        H_feature = Upsample(H_feature, size=L_feature.size()[2:], align_corners=False)

        # 特征融合使用 torch.cat 之前确保特征图尺寸相同
...
class PrewittConv(nn.Module):
    def __init__(self, channel, return_grad=False):
        """
        :param channel: 输入通道数
        :param return_grad: 是否返回梯度方向信息（默认返回幅值）
        """
        super().__init__()
        ...
        # 构建可分离卷积核
        self._build_conv_layers(channel, kernel_x, kernel_y)

    def _build_conv_layers(self, channel, kernel_x, kernel_y):
        # X方向卷积
        self.conv_x = nn.Conv2d(
            channel, channel, kernel_size=3,
            padding=1, groups=channel, bias=False
        )
        self.conv_x.weight.data = torch.tensor(kernel_x).unsqueeze(0).unsqueeze(0).expand(channel, 1, 3, 3)
        ...
        self.conv_y.weight.data = torch.tensor(kernel_y).unsqueeze(0).unsqueeze(0).expand(channel, 1, 3, 3)
    ...
    def forward(self, x):
        ...
        if self.return_grad:
            return torch.cat([gx, gy], dim=1)  # 返回双通道梯度信息
        return torch.sqrt(gx ** 2 + gy ** 2 + 1e-6)  # 返回梯度幅值


class LaplacianConv(nn.Module):
    def __init__(self, channel, kernel_type='8neighbor', return_abs=True):
        """
        :param channel: 输入通道数
        :param kernel_type: 卷积核类型 ['4neighbor', '8neighbor']
        :param return_abs: 是否返回绝对值（推荐保持True）
        """
        super().__init__()
        self.return_abs = return_abs

        # 定义Laplacian核
        if kernel_type == '4neighbor':
            kernel = np.array([[0, 1, 0],
                               [1, -4, 1],
                               [0, 1, 0]], dtype=np.float32)
        elif kernel_type == '8neighbor':
            kernel = np.array([[1, 1, 1],
                               [1, -8, 1],
                               [1, 1, 1]], dtype=np.float32)
        else:
            raise ValueError("Unsupported kernel type. Choose from ['4neighbor', '8neighbor']")
        ...
        self.conv.weight.data = torch.tensor(kernel).unsqueeze(0).unsqueeze(0).expand(channel, 1, 3, 3)
        self.conv.weight.requires_grad_(False)

    def forward(self, x):
        output = self.conv(x)
        return torch.abs(output) if self.return_abs else output


class RobertsConv(nn.Module):
    def __init__(self, channel):
        ...
        # 创建卷积核张量
        kernel_x = torch.tensor(roberts_x).unsqueeze(0).unsqueeze(0)  # (1,1,2,2)
        kernel_x = kernel_x.expand(channel, 1, 2, 2)  # (channel,1,2,2)

        kernel_y = torch.tensor(roberts_y).unsqueeze(0).unsqueeze(0)
        kernel_y = kernel_y.expand(channel, 1, 2, 2)

        # 创建卷积层
        self.conv_x = nn.Conv2d(channel, channel, kernel_size=2,
                                groups=channel, bias=False)
        self.conv_y = nn.Conv2d(channel, channel, kernel_size=2,
                                groups=channel, bias=False)
    ...
    def forward(self, x):
        # 输入填充保证输出尺寸不变
        x_pad = F.pad(x, (0, 1, 0, 1), mode='replicate')  # 右、底部各填充1像素
        return self.conv_x(x_pad) + self.conv_y(x_pad)




class LEEB(nn.Module):
    def __init__(self, inc, ouc) -> None:
        super().__init__()

        self.sobel_branch = SobelConv(inc)
        # self.sobel_branch = RobertsConv(inc)
        self.conv_branch = DWConv(inc, inc, 3)
...
class C2f_LEEB(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(LEEB(self.c, self.c) for _ in range(n))


class MREM(nn.Module):
    def __init__(self, in_channels, reduction=16, scales=(3, 5, 7)):
        super(MREM, self).__init__()
        self.scales = scales
        self.convs = nn.ModuleList([
            nn.Conv2d(in_channels, in_channels, k, padding=k // 2)
            for k in scales
        ])
        self.fuse = nn.Conv2d(in_channels * len(scales), in_channels, 1)
    ...
    def forward(self, x):
        ...
        # 叠加操作：将多尺度特征图在通道维度上进行拼接
        combined_features = torch.cat(multi_scale_features, dim=1)

        # 使用卷积层整合叠加后的特征
        fused_features = self.fuse(combined_features)

        # 全局平均池化并重塑
        b, c, _, _ = fused_features.size()
        avg_out = self.avg_pool(fused_features).view(b, c)

        # 全连接层和sigmoid激活函数计算注意力权重
        fc_out = self.fc(avg_out).view(b, c, 1, 1)

        # 应用注意力权重
        out = fused_features * fc_out.expand_as(fused_features)

        # 残差连接
        identity_x = x.clone().detach()
        out += identity_x
